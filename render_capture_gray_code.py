...
"""
Main file for rendering gray code patterns, and capturing images of the 
patterns.
"""
...
class GrayCodeController():
    def __init__(self, full_screen, pos_neg_codes, capture_images,
                 display_captured_images, horizontal, vertical):
        app = wx.App(False)
        self.app = app
        ...
        if not full_screen:
            ...
            frame = wx.Frame(
                None, -1, "Gray code projection", size=self.window_size)
        else:
            self.window_size = wx.GetDisplaySize()
            frame = wx.Frame(
                None, -1, "Gray code projection", size=self.window_size,
                style=wx.NO_BORDER)
            frame.Maximize(True)
        # Add a panel to the frame, -1 is default ID
        self.gray_code_panel = GrayCodePanel(frame, -1, self.window_size)
        # Show the frame
        frame.Show(True)
        ...
        if capture_images:
            ...
            camera_frame = wx.Frame(
                None, -1, "Camera capture", size=self.window_size)
            self.camera_panel = GrayCodeCameraPanel(
                camera_frame, capture, capture_library)
        ...
        self.gray_code_state = GrayCodeState(self.window_size)

        self.iteration = 0
        self.black_iteration = 0
        self.white_iteration = 1
        self.first_gray_code_iteration = 2
        self.last_gray_code_iteration = self.first_gray_code_iteration \
            + self.gray_code_state.get_num_bit_planes()

        self.horizontal = horizontal
        self.vertical = vertical
        self.horizontal_iterations = self.horizontal
        assert(self.horizontal or self.vertical)

        # Whether to render and capture both positive and negative gray codes, 
        # for robustness.
        # A positive frame has 0 represented as black, and 1 as white. A 
        # negative frame is the opposite.
    ...
    def processing_loop(self, event):
        ...
        if self.iteration == self.black_iteration:
            ...
            if self.is_render:
                ...
                # image = Image.fromarray(gray_image)
                print("Captured white image:", image)
                # image = image.convert('gray')
            ...
            if self.is_render:
                print("Rendering gray code pattern %d of %d." %
                      (self.iteration - self.first_gray_code_iteration,
                       self.last_gray_code_iteration - self.first_gray_code_iteration))
                self.gray_code_panel.render_black = False
                self.gray_code_panel.render_white = False
                # Rendering a Gray code pattern.

                self.gray_code_panel.horizontal = self.horizontal_iterations
                ...
                print("Capturing gray code pattern %d of %d." %
                      (self.iteration - self.first_gray_code_iteration,
                       self.last_gray_code_iteration - self.first_gray_code_iteration))
                if self.iteration < self.last_gray_code_iteration:
                    ...
                    if self.horizontal_iterations:
                        direction_label = "horiz"
                    else:
                        direction_label = "vert"

                    if self.pos_neg_codes:
                        pos_neg_label = "pos" if self.is_pos_frame else "neg"
                        image.save("graycode_%02d_%s_%s.png" %
                                   (self.iteration - self.first_gray_code_iteration,
                                    direction_label, pos_neg_label))
                    else:
                        image.save("graycode_%02d_%s.png" %
                                   (self.iteration - self.first_gray_code_iteration,
                                    direction_label))
                else:
                    # We've reached the end of the sequence.
                    print("Reached end of sequence.")
                    if self.horizontal_iterations and self.vertical:
                        # Proceed to vertical gray code.
                        self.horizontal_iterations = False
                        self.iteration = self.first_gray_code_iteration
                        self.is_render = True
                        self.gray_code_state = GrayCodeState(self.window_size)
                        if self.pos_neg_codes:
...
class GrayCodePanel(wx.Panel):
    ...
    def render_gray_code(self, state, is_pos_frame):
        ...
        if is_pos_frame:
            print("Pos background.")
            self.SetBackgroundColour("black")
        else:
            print("Neg background.")
            self.SetBackgroundColour("white")
        ...
        if is_pos_frame:
            print("Pos foreground.")
            dc.SetPen(wx.Pen("white"))
            dc.SetBrush(wx.Brush("white"))
        else:
            print("Neg foreground.")
...
if __name__ == '__main__':
    ...
    controller = GrayCodeController(
        full_screen, pos_neg_codes, capture_images, display_captured_images,
